...
export class SolosLeaderboardController {
	...
	async getSolosLeaderboardByChannelId(
		...
		return await this._solosLeaderboardService.getSolosLeaderboardByChannelId(channelId, skip);
	}

	@Get("/guild/:guildId")
	async getSolosLeaderboardsByGuildId(@Param("guildId") guildId: string, @Query("skip", ParseIntPipe) skip: number) {
		return await this._solosLeaderboardService.getSolosLeaderboardsByGuildId(guildId, skip);
	}
}
